...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        int longestBurstTime = INT_MIN;
        for (int j = i; j < n; ++j) {
            if (processes[j].arrivalTime <= currentTime && processes[j].burstTime > longestBurstTime) {
                longestJobIndex = j;
                longestBurstTime = processes[j].burstTime;
            }
        }
        if (longestJobIndex == -1) {
            break;
        }
        swap(processes[i], processes[longestJobIndex]);
